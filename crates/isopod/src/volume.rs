...
use crate::constants::{SECTOR_SIZE, ISO_STANDARD_ID, volume_type};
            ...
            if offset + 7 <= buffer.len() &&
               ...
               &buffer[offset+1..offset+6] == ISO_STANDARD_ID {
                ...
                let path_table_location_l = u32::from_le_bytes([
                    buffer[offset+140], buffer[offset+141], buffer[offset+142], buffer[offset+143]
                ]);
                let optional_path_table_location_l = u32::from_le_bytes([
                    buffer[offset+144], buffer[offset+145], buffer[offset+146], buffer[offset+147]
                ]);
                let path_table_location_m = u32::from_be_bytes([
                    buffer[offset+148], buffer[offset+149], buffer[offset+150], buffer[offset+151]
                ]);
                let optional_path_table_location_m = u32::from_be_bytes([
                    buffer[offset+152], buffer[offset+153], buffer[offset+154], buffer[offset+155]
                ]);
                
                // Parse root directory entry
                let root_directory_entry = DirectoryEntry::parse_from_buffer(
                    &buffer[offset+156..offset+156+34]
...
impl VolumeDescriptor for PrimaryVolumeDescriptor {
    fn write_to<W: Write + Seek>(&self, writer: &mut W) -> Result<()> {
        ...
        self.root_directory_entry.write_to_buffer(&mut buffer[156..190])?;
